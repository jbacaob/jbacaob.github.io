...
In the language of Markov decision problems:
...
has the nice property that $$T^kv\rightarrow v^*$$ as $$k\rightarrow \infty$$ for any $v \in \mathbb{R}^{\mathcal{X}}$. Moreover, $v^*$ is the unique solution of the Bellman equation above.
...
$$
\lVert v(k) - w(k) \rVert_\infty = \sup_{k\in X}\{v(k) - w(k)\}
$$

In this case, it is possible to prove that $T$ has a unique fixed point $$v^*$$ on $$\mathbb{R}^{\mathcal{X}}$$ which is also globally stable. The fact that $$v^*$$ is the unique solution of the Bellman equation is not hard to check. A thorough discussion of Markov decision problems, accompanied by a detailed proof of the aforementioned facts can be found at [^1].

{: .prompt-tip }

Our goal now is to introduce two techniques to find $$v^*$$. The first technique, value function iteration (VFI), is widely known in economics. The second technique, a form of reinforcement learning called _Q-learning_, is relatively less known.

## Value function iteration

This algorithm is a ubiquitous dynamic programming technique in which we turn the Bellman equation \eqref{bellman} into an updating rule, in the spirit of the Bellman operator.

For the deterministic optimal growth model, the steps of the algorithm are as follows:
...
The choice of tolerance means that we can get arbitrarily closer to the value function $$v^*$$. This algorithm must end. As we argued in the previous section, the Bellman operator $Tv$ is a contraction and converges to its unique fixed point $$v^*$$ up to some approximation error. 

From now on, we adopt a Cobb-Douglas functional form for $f$ with output-capital elasticity $\alpha$. For the instantaneous utility function $u$, let's use a specification with constant relative risk aversion $\sigma \neq 1$. If $$\sigma = 1$$ then $$u(c) = \ln c$$. 

$$
f(k) = k^\alpha,\quad u(c) = \frac{c^{1-\sigma}-1}{1-\sigma}
$$

The Python code below allows us to find $$v^*$$. We employ Numba to speed up computations.
...
Q-learning is an example of reinforcement learning algorithm used to find the optimal action-selection policy
for a given finite Markov decision process (MDP), together with the associated optimal value. 
More specifically, it's a type of _temporal difference_ learning, in the sense that it can learn 
directly from the environment without an underlying set of dynamics, at the same time that it updates 
estimates based on other previously learned estimates. In other words, Q-learning puts together
Monte Carlo and dynamic programming techniques.

Given our MDP for the deterministic optimal growth model, let's introduce the notion of a $Q$-value. A $Q$-value is
the expected reward the social planner obtains by implementing action $k'$ and state $k$. Because of the law of motion for capital,
...
Similarly VFI, the goal of Q-learning is to compute the action-state value function $$q^*(k,k')$$, which comprises the Q-values of the optimal policy. 
It is clear that:
...
which means that by finding the optimal $q$-values we also learn about the value function. 
...
sum of the learning rates for each state-action pair $$(k, k')$$ diverges over time, while the sum of their squares remains finite.
{: .prompt-tip }
...
An excellent introduction to reinforcement learning techniques and Q-learning is presented in [^2].
...
The plot below compares the performances of the VFI and Q-learning techniques. For the latter, we employ
an $$\varepsilon$$-greedy action selection with $$\varepsilon = 0.1$$, while varying the number of episodes between 1000 and 100000.

![QL](/assets/img/blog9/ql.svg){: w = "100"}

We observe that the higher the number of episodes, the closer the implied results of the Q-learning algorithm are to the solution of the
value function iteration. The planner effectively learns the optimal q-values by exploring and exploiting more when the number of 
episodes is large enough.

### Log preferences

Now, suppose that the social planner receive an instantaneous, logarithmic utility of consumption $$u(c) = \ln(c)$$.
In this case, the coefficient of relative risk aversion is exactly one, i.e., $$\sigma = 1$$. Let's also assume that current capital fully depreciates every period, which means that $$\delta = 1$$. With these simplifications, an explicit solution for the value function exists and is given by

$$
V(k) = \frac{1}{1-\beta}\left[\ln (1-\alpha \beta) + \frac{\alpha \beta}{1-\alpha \beta}\ln(\alpha \beta)\right] + \frac{\alpha}{1-\alpha \beta} \ln k
$$

When the number of episodes in the Q-learning method is set to 100000, both VFI and Q-learning yield functions that are virtually indistiguishable from the true value function.
